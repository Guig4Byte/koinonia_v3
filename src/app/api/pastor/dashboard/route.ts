...
export async function GET(request: Request) {
  try {
    ...
    // Busca todos os grupos da igreja
    ...
    // Busca nomes de líderes e supervisores
    ...
    const users = await prisma.user.findMany({
      where: { id: { in: [...leaderIds, ...supervisorIds] } },
    ...
    // Busca tasks vencidas por líder
    const now = new Date();
    ...
    const groupSummaries = groups.map((group) => {
      ...
      // Presença média (últimos 6 eventos)
      ...
      // Alerta: queda de presença (se último evento passado teve 0% ou menos de 50%)
      ...
      // Alerta: presença não registrada no evento mais recente que já ocorreu
      if (pastEvents.length > 0) {
        const latestEvent = pastEvents[0]!;
        if (
          latestEvent.attendances.length === 0 &&
      ...
      return {
        ...
        lastAttendanceRate:
          pastEvents.length > 0 && pastEvents[0]!.attendances.length > 0
            ...
            : null,
        supervisorName: group.supervisorId ? userNameMap.get(group.supervisorId) ?? null : null,
        leaderName: group.leaderId ? userNameMap.get(group.leaderId) ?? null : null,
      };
    });
    ...
    return NextResponse.json({
      ...
      alerts: alerts.slice(0, 10), // Máximo 10 alertas
    });
  } catch (error) {
