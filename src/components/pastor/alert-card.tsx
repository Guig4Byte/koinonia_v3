...
import type { PastorDashboardAlert } from "@/hooks/use-pastor-dashboard"
...
export function AlertCard({ alert }: { alert: PastorDashboardAlert }) {
  ...
  const href = alert.personId
    ...
    : alert.groupId
    ? `/pastor/celulas/${alert.groupId}`
    : undefined

  const className = cn(
    "flex items-start gap-3 rounded-xl border-l-4 p-3 transition hover:opacity-80",
    config.color
  )
